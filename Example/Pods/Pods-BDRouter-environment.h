
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// BDRouter
#define COCOAPODS_POD_AVAILABLE_BDRouter
#define COCOAPODS_VERSION_MAJOR_BDRouter 0
#define COCOAPODS_VERSION_MINOR_BDRouter 1
#define COCOAPODS_VERSION_PATCH_BDRouter 0

// Underscore.m
#define COCOAPODS_POD_AVAILABLE_Underscore_m
#define COCOAPODS_VERSION_MAJOR_Underscore_m 0
#define COCOAPODS_VERSION_MINOR_Underscore_m 2
#define COCOAPODS_VERSION_PATCH_Underscore_m 1

